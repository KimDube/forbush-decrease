...
def find_mean_gcr(epoch_starts, plot=0, shifted=0):
    ...
    if plot == 1:
        ...
        plt.tight_layout()
        plt.savefig('/home/kimberlee/Masters/ForbushDecrease/plots/MOSC_surf.png', dpi=300)
        plt.show()
        ...
        plt.plot(PLOT_RANGE, gcr_mean, 'r')
        plt.plot(PLOT_RANGE, gcr_mean + (gcr_std / np.sqrt(NUM_EVENTS)), 'b', linewidth=0.5)
        plt.plot(PLOT_RANGE, gcr_mean - (gcr_std / np.sqrt(NUM_EVENTS)), 'b', linewidth=0.5)
        ...
        plt.xlabel("Day from Event")
        plt.title("Forbush Decreases: 2002-15")
        plt.savefig('/home/kimberlee/Masters/ForbushDecrease/plots/MOSC_mean_signif.png', dpi=300)
        plt.show()
...
if __name__ == "__main__":
    ...
    EVENTS = ['2002-02-04', '2002-03-18', '2002-04-17', '2002-05-23', '2002-08-18', '2003-01-26', '2003-05-29',
              '2003-08-17', '2003-10-30', '2003-11-20', '2004-07-26', '2004-09-13', '2004-11-09', '2005-01-18',
              '2005-05-15', '2005-07-17', '2005-09-11', '2006-07-09', '2006-12-14', '2007-01-29', '2010-08-03',
              '2011-04-11', '2011-06-22', '2011-08-05', '2011-10-24', '2012-01-22', '2012-03-08', '2012-04-04',
    ...
    #                                 version='6.00', start_date='2002-01-01', end_date='2016-12-31')

    # x.to_netcdf(path='/home/kimberlee/Masters/OMPS_all', mode='w')
    # exit()
    # x = xarray.open_dataset('/home/kimberlee/Masters/OMPS_all')

    # OMPS: computer can't handle saving
    ...
    comps = np.zeros(EPOCH_LENGTH)
    for i in range(EPOCH_LENGTH):
        compm[i] = np.nanmedian(eventarr[i, :])
        comps[i] = np.nanstd(eventarr[i, :])

    NUM_EVENTS = events_with_data
    ...
    plt.plot(PLOT_RANGE, compm, 'r')
    plt.plot(PLOT_RANGE, aer_mean_err + 1.96 * aer_std_err, 'k-.')
    plt.plot(PLOT_RANGE, aer_mean_err - 1.96 * aer_std_err, 'k-.')
    plt.plot(PLOT_RANGE, compm + (comps / np.sqrt(NUM_EVENTS)), 'b', linewidth=0.5)
    plt.plot(PLOT_RANGE, compm - (comps / np.sqrt(NUM_EVENTS)), 'b', linewidth=0.5)

    plt.ylabel("OMPS Aerosol Extinction Anomaly")
    plt.xlabel("Day from Event")
    plt.title("2002-15, %i km, tropics" % alt_of_interest)
    plt.savefig('/home/kimberlee/Masters/ForbushDecrease/OMPS_%ikm' % alt_of_interest, dpi=300)
    plt.show()
